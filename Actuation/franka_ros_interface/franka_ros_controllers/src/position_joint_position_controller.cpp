...
bool PositionJointPositionController::init(hardware_interface::RobotHW* robot_hardware,
  ...
  std::map<std::string, double> accel_limit_map;
  ...
  if (!node_handle.getParam("/robot_config/joint_config/joint_acceleration_limit", accel_limit_map) ) {
  ROS_ERROR(
      "PositionJointPositionController: Joint limits parameters not provided, aborting "
      "controller init!");
  return false;
  ...
  for (size_t i = 0; i < joint_limits_.joint_names.size(); ++i){
    ...
    if (vel_limit_map.find(joint_limits_.joint_names[i]) != vel_limit_map.end())
      ...
      {
        ROS_ERROR("PositionJointPositionController: Unable to find velocity limit  values for joint %s...",
                       joint_limits_.joint_names[i].c_str());
      }
      
    if (accel_limit_map.find(joint_limits_.joint_names[i]) != accel_limit_map.end())
      {
        joint_limits_.accel.push_back(accel_limit_map[joint_limits_.joint_names[i]]);
      }
      else
      {
        ROS_ERROR("PositionJointPositionController: Unable to find acceleration limit  values for joint %s...",
                       joint_limits_.joint_names[i].c_str());
      }
  }  
  ...
  for (size_t i = 0; i < 7; i++) {
    maxVelocity[i] = joint_limits_.velocity[i];
    maxAccel[i] = joint_limits_.accel[i];
  }
...
void PositionJointPositionController::update(const ros::Time& time,
  ...
  // pos_d_ = franka::limitRate(maxVelocity, maxAccel, franka::kMaxJointJerk, pos_d_, prev_pos_d, vel_d_, accel_d_);
  // if (period.toSec() > 0) {
  //   updateDerivatives(period.toSec());
  // }
