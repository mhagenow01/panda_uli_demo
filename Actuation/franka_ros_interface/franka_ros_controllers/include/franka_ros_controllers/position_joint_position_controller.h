...
class PositionJointPositionController : public controller_interface::MultiInterfaceController<
 ...
 private:
  ...
  std::array<double, 7> prev_pos_d{};
  std::array<double, 7> pos_d_;
  std::array<double, 7> vel_d_{};
  std::array<double, 7> accel_d_{};

  std::array<double, 7> maxVelocity{};
  std::array<double, 7> maxAccel{};
  ...
  void updateDerivatives(double dt) {
    for (size_t i = 0; i < 7; i++) {
      double v = (pos_d_[i] - prev_pos_d[i]) / dt;
      accel_d_[i] = (v - vel_d_[i]) / dt;
      vel_d_[i] = v;
      prev_pos_d[i] = pos_d_[i];
    }
  }
};
