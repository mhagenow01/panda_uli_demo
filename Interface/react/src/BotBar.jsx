...
export const BotBar = (props) => {  
  // const [connect, connection, url,imageTopic,setCanvasOpacity] = useRosStore(state=>([state.connect, state.connection, state.url,state.imageTopic,state.setCanvasOpacity])) 
  ...
  // const [messages, addMessage, sendMessage, sendCoordinates,publishStates] = useAppStore(state=>([state.messages,state.addMessage,state.sendMessage,state.sendCoordinates,state.publishStates]))
    return (
      <Box height='100%' width='100%'>
        <Button onClick={()=>{
        }} size="large" alignSelf="center" label="Reset"></Button>
      </Box>
    //   <Grid
    //   rows={['10%', '10%','70%']}
    //   columns={['90%', '10%']}
